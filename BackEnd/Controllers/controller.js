const { testModel, getAllSumModel, addItemModel, getItemsModel, updateItemModel, deleteItemModel } = require("../Models/itemModel");
...
const addItemController = async (req, res) => {
    try {
        ...
        if (!item.title || !item.description) {
            return res.status(400).json({ statusCode: 400, message: 'Inappropriate data provided' });
        }
        const response = await addItem(item)
        return res.status(201).json({ statusCode: 201, message: 'Item added successfully', data: response.insertedId });
    } catch (error) {
...
const getAllItemsController = async (req, res) => {
    try {
        ...
        return res.status(200).json({statusCode: 201, data, message: 'fetched all items successfully'});
    } catch (error) {
...
const updateItemController = async (req, res) => {
    try {
        ...
        return res.status(200).json({ statusCode: 200, message: 'Item updated successfully' });
    } catch (error) {
...
const deleteItemController = async (req, res) => {
    try {
        ...
        return res.status(200).json({ statusCode: 200, message: 'Item deleted successfully' });
    } catch (error) {
...
module.exports = { testController, getAllSums, addItemController, getAllItemsController, updateItemController, deleteItemController }